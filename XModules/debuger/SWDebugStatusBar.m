...
#import "SWDebugStatusBar.h"
#import "SWDebug.h"
#import <objc/runtime.h>
#import "SWDebugSysCtl.h"
...
@interface SWDebugMemoryView : UIView
{
    NSMutableArray *mPointList;
    int mPCount;
    drawLines *mdraw;
}
@property (assign, nonatomic) SWMemroyInfo memInfo;
@end

@interface SWDebugCpuView : UIView
{
...
@interface SWDebugStatusBar ()
@property (strong, nonatomic) UIButton *debugButton;
@property (strong, nonatomic) SWDebugMemoryView *memoryView;
@property (strong, nonatomic) SWDebugCpuView *cpuView;
@property (strong, nonatomic) UIWindow *window;
@property (strong, nonatomic) UIView *statusbar;

@end

@implementation SWDebugStatusBar
IMP_SINGLETON
...
+ (void)attach
{
    ...
    SWDebugStatusBar *bar = [SWDebugStatusBar sharedInstance];
    bar.frame = frame;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.statusbar=[SWDebug sysStatusBar];
...
//        [self.statusbar addGestureRecognizer:tapPress];
        ...
        //内存视图
        self.memoryView = [[SWDebugMemoryView alloc] initWithFrame:CGRectZero];
        //self.memoryView.alpha=0.6;
        [self.statusbar insertSubview:self.memoryView atIndex:0];
        
        //cpu视图
        self.cpuView=[[SWDebugCpuView alloc] initWithFrame:CGRectZero];
...
- (float)getDebugButtonRight
{
    ...
    if (self.statusbar) {
        ...
        Class clazz3=[SWDebugCpuView class];
        for (UIView *v in vs) {
...
- (void)layoutSubviews
{
    ...
    if (self.statusbar!=[SWDebug sysStatusBar]) {
        self.statusbar=[SWDebug sysStatusBar];
    ...
    self.cpuView.frame = CGRectMake(250, 0, self.width-250, 20);
    self.memoryView.frame = CGRectMake(0, 0, self.width, 1);
...
//    DebugerTableViewController *controller = [[DebugerTableViewController alloc] initWithStyle:UITableViewStylePlain];
//    UIBarButtonItem *item = [[UIBarButtonItem alloc] initWithTitle:@"Close" style:UIBarButtonItemStyleBordered target:self action:@selector(onCloseDebugger:)];
...
- (void)onUpdateInfo:(NSTimer *)timer
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        while (1) {
            self.memoryView.memInfo = [SWDebug memoryInfo];
            [self.cpuView setUsage:[SWDebugSysCtl cpuUsage]];
...
@implementation SWDebugMemoryView
-(id)initWithFrame:(CGRect)frame{
    self=[super initWithFrame:frame];
    if (self) {
...
//设置当前内存占用率
- (void)setMemInfo:(SWMemroyInfo)info
{
    _memInfo = info;
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (_isLongPress) {
        ...
        //正常按下，线条显示
        SWMemroyInfo info = self.memInfo;
        natural_t total = info.free + info.used;
        float percent = (float)info.used / total;

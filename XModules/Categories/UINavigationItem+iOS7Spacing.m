...
- (BOOL)needAddSpace
{
    return self.autoAddSpace&&[[[UIDevice currentDevice] systemVersion] floatValue]>=7.f;
}

- (BOOL)autoAddSpace
{
    id b=objc_getAssociatedObject(self, &"autoAddSpace");
    if (!b) {
        return YES;
    }
    return [b boolValue];
...
- (void)setAutoAddSpace:(BOOL)autoAddSpace
{
    objc_setAssociatedObject(self, &"autoAddSpace", @(autoAddSpace), OBJC_ASSOCIATION_RETAIN);
}
...
{
    if ([self needAddSpace] && [[leftBarButtonItems firstObject] customView] && leftBarButtonItems.count > 0) {
...
{
    if ([self needAddSpace] && rightBarButtonItem.customView) {
...
{
    if ([self needAddSpace] && [[rightBarButtonItems firstObject] customView] && rightBarButtonItems.count > 0) {
